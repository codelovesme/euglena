# 19 — The comment marker is `|`, and the baseline is code 1.4.0
...
`code` 1.4.0 replaces the `--` line comment with `|`. One character, because
...
Under `code` 1.4.0 the old spelling of those lines does not parse — `--`
...
`MIN_CODE_VERSION` goes **1.1.0 → 1.4.0**. It is the version euglena's
...
|---|---|---|
| `MIN_CODE_VERSION_FOR_TEST` | 1.2.0 (`code test`) | below 1.4.0 |
| `MIN_CODE_VERSION_FOR_UNINSTALL` | 1.3.0 (`code uninstall`, T18) | below 1.4.0 |

No `code` can satisfy a 1.4.0 baseline and still be too old for either
...
its manifest now asks for `>=1.4.0`.
