...
exports.default = core_1.vacuole.v1.com({
    ...
    GetAlive: function (p, _a) {
        var cp = _a.cp, t = _a.t;
        var host = sap.host, port = sap.port, database = sap.database;
        return new Promise(function (resolve) {
            mongodb_1.MongoClient.connect("mongodb://" + host + ":" + port, { useNewUrlParser: true, useUnifiedTopology: true }, function (err, _db) {
                if (!err) {
                    db = _db.db(database);
    ...
    ReadParticle: function (p, _a) {
        ...
        return new Promise(function (resolve) {
            ...
            db.collection("particles")
                .find(cessnalib_1.js.Class.toDotNotation(query))
    ...
    SaveParticle: function (p, _a) {
        ...
        return new Promise(function (resolve) {
            if (p.data instanceof Array) {
                db.collection("particles").insertMany(p.data, function (err) {
                    if (err)
                        return resolve(cp.Exception({ message: JSON.stringify(err) }));
                    return resolve(cp.ACK());
                });
                ...
                if (query) {
                    if (count === "all") {
                        return db
                            ...
                            .updateMany(cessnalib_1.js.Class.toDotNotation(query), particle, { upsert: true }, function (err, doc) {
                            if (err)
                                return resolve(cp.Exception({ message: JSON.stringify(err) }));
                            return resolve(cp.ACK());
                        });
                        ...
                        return db
                            .collection("particles")
                            .findOneAndUpdate(cessnalib_1.js.Class.toDotNotation(query), particle, { upsert: true }, function (err, doc) {
                            if (err)
                                return resolve(cp.Exception({ message: JSON.stringify(err) }));
                            return resolve(cp.ACK());
                        });
    ...
    RemoveParticle: function (p, _a) {
        ...
        return new Promise(function (resolve) {
            ...
            if (count === "all") {
                db.collection("particles").remove(cessnalib_1.js.Class.toDotNotation(query), function (err, doc) {
                    if (err)
                        return resolve(cp.Exception({ message: JSON.stringify(err) }));
                    return resolve(cp.ACK());
                });
